...
对受变更状态约束的 component query，统一使用 `query(...)`、`each(...)` 或 `queryState(...)` 的 `added` / `changed` 过滤器。在原地修改 component 值之后，使用 `markComponentChanged(...)` 标记变更。
...
如果只想匹配新加入的 component，使用同样形式的 `{ added: [Position] }`。
